...
- ext_code = kode eksternal item dari sistem lama
...
function formatQueryResult({ tables, relationHints, sql, rows, totalRows }) {
  ...
  const lines = [
    ...
    ``,
    truncated
      ? `⚠️  Ditampilkan ${MAX_ROWS} dari ${totalRows} baris (gunakan filter untuk data lebih spesifik)`
...
// ============================================================
//  DOCS HELPER — baca file .md dari folder docs/
// ============================================================

/** Rekursif scan semua file .md di dalam dir */
async function scanMarkdownFiles(dir) {
  ...
  try {
    ...
    return results; // folder tidak ada, return kosong
  }
...
/** Cari keyword di semua file .md, return potongan konteks */
async function searchDocs(keyword) {
  ...
  for (const filePath of files) {
    ...
    // Ambil baris yang mengandung keyword + 2 baris konteks
  ...
  try {
    ...
    if (toolName === "baca_dokumen") {
      ...
      if (!fullPath.endsWith(".md")) {
        return { content: [{ type: "text", text: "❌ Hanya file .md yang bisa dibaca." }] };
      }
